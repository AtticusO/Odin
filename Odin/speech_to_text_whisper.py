...
Drop-in replacement for speech_to_text.py using OpenAI Whisper for local,
offline transcription with significantly better accuracy.
...
Whisper model sizes (tradeoff: speed vs accuracy):
    ...
    'base.en' is a good default — fast and accurate for English.

Functions:
    transcribe_from_microphone(...) -> str
    transcribe_audio(audio_np)     -> str
"""
...
def _get_model(model_name: str = "base.en") -> whisper.Whisper:
    global _model
    if _model is None:
...
def _record_fixed(duration: float) -> np.ndarray:
    ...
    **_kwargs,  # accepts unused args for drop-in compatibility
    ...
    if duration is not None:
        ...
        audio = _record_until_silence(
            ...
            phrase_time_limit=phrase_time_limit or 10.0,
        )
        if audio is None:
            return "wait_timeout"
